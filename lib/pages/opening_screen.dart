...
import 'package:hidden_gem_vtuber_singing_relay_19th/utils/string.dart';
import 'package:hooks_riverpod/hooks_riverpod.dart';
import 'package:pointer_interceptor/pointer_interceptor.dart';
import 'package:responsive_framework/responsive_framework.dart';
import 'package:youtube_player_iframe/youtube_player_iframe.dart';

class OpeningScreen extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final videoId = YoutubePlayerController.convertUrlToId(opening.url)!;
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              itemCount: opening.timelines.length,
              itemBuilder: (context, index) {
                final timeline = opening.timelines[index];
                return ListTile(
                  title: Text(timeline.title),
                  trailing: Text(timeline.time),
                  onTap: () {
                    controller.seekTo(seconds: timeline.time.toSeconds(), allowSeekAhead: true);
                  },
                );
              },
            ),
